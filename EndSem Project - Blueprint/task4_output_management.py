...
TASK 4 ▶ Output Management & Reporting
  ...
  CHANGES (this version):
    ...
    3. TARGET STRING FIX
       Uses numeric constants for Tg and Dk evaluations to avoid math errors
       when Task 3 passes formatted strings instead of floats.
"""
...
def merge_morgan_fingerprints(master_df: pd.DataFrame, morgan_df: pd.DataFrame) -> pd.DataFrame:
    """Merge Morgan fingerprints generated in Task 5 into the master dataset."""
    merge_cols = ["Polymer", "Sample_ID"]
    if "Sample_ID" not in master_df.columns or "Sample_ID" not in morgan_df.columns:
        # Fallback if no Sample_ID
        return pd.concat([master_df, morgan_df[[c for c in morgan_df.columns if c.startswith("morgan_")]]], axis=1)
    return pd.merge(master_df, morgan_df, on=merge_cols, how="left")

def save_polymer_csvs(enriched_df: pd.DataFrame,
                      feature_cols: list) -> list:
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    target_cols = ["Tg_degC", "Dk_1GHz"]
    all_cols    = feature_cols + target_cols  # SMILES removed from here
    saved_paths = []
...
def print_inverse_design_report(results: list) -> None:
    ...
    for r in results:
        # FIX: Replaced r.get() with numeric constants to prevent string errors
        print(f"    {r['polymer']:<12}: "
              f"Tg >= {LEO_TG_TARGET:.0f} C   "
    ...
    for r in results:
        # FIX: Replaced r.get() with numeric constants to prevent string errors
        tg_ok = "OK" if r["predicted_Tg_degC"] >= LEO_TG_TARGET else "X"
        dk_ok = "OK" if r["predicted_Dk"]      <= LEO_DK_TARGET else "X"
...
def compute_confidence(metrics, inv_results, feature_cols, leakage, physics_report):
    ...
    n_both = sum(1 for r in inv_results
                 if (r["predicted_Tg_degC"] >= LEO_TG_TARGET
                     and r["predicted_Dk"]  <= LEO_DK_TARGET))
