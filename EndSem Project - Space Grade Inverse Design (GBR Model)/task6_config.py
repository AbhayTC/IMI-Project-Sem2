...
TASK 6 ▶ Interactive Export Configuration
  ...
  Inputs : master_dataset.pkl, morgan_fingerprints.pkl
...
#  IMPORTS
# ──────────────────────────────────────────────────────────────────────────────
import warnings, pickle, sys, json, os
...
# task4 will use only the top N features per block from this config.
# Set here so that the interactive prompt shows a meaningful default.
TOP_N_PER_BLOCK = 5
...
# NOTE: The polymer list for within-class variance ranking is derived
# dynamically from master_df["Polymer"].unique() filtered to is_reference=True
# rows.  No hard-coded list is needed here.
...
def rank_features_by_within_class_variance(df: pd.DataFrame,
    ...
    feature_cols: column names to rank
    polymers    : list of polymer names to include (derived from df at call
                  site; includes all 6 reference polymers plus any user polymer)
    """
...
def rank_all_blocks(df: pd.DataFrame,
    ...
    The polymer list used for within-class variance is derived from reference
    rows in df so that all 6 reference polymers (and any user polymer present
    as a non-dummy row) are included automatically.
    """
    # Derive polymer list from reference rows (is_reference=True when present)
    if "is_reference" in df.columns:
        ref_df = df[df["is_reference"] == True]
    else:
        ref_df = df
    # Exclude dummy rows if column exists
    if "is_dummy" in ref_df.columns:
        ref_df = ref_df[ref_df["is_dummy"] == False]
    polymers = sorted(ref_df["Polymer"].dropna().unique().tolist())
    print(f"  Polymer list for within-class variance: {polymers}")
    ...
    for key in ("structural", "latent", "physics"):
        ranked[key] = rank_features_by_within_class_variance(
            df, BLOCK_DEFS[key]["cols"], polymers
        )

    # Morgan from morgan_df
    if morgan_df is not None:
        ...
        for col in mfp_cols:
            ...
            per_poly_var = [
                ...
                for p in polymers
                if p in morgan_df["Polymer"].values
            ]
...
def run_interactive_config(df: pd.DataFrame,
    ...
    print(f"\n  {YELLOW}ℹ  task4_output_management.py uses the top {TOP_N_PER_BLOCK} features per block")
    print(f"     in consolidated CSVs for scannability.  You may select up to the")
    print(f"     block maximum here; task4 will slice the first {TOP_N_PER_BLOCK} automatically.{RESET}\n")
...
if __name__ == "__main__":
    ...
    out_config = build_config(chosen, ranked)
    print_config_summary(out_config)
