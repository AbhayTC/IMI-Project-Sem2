...
================================================================================
  ...
  Polymers: Polyimide (PI) | PEEK | PTFE | CyanateEster | FluorinatedPolyimide | Polybenzoxazole
================================================================================
...
TASK 5 ▶ Morgan Fingerprint Generation (ECFP4, radius=2, nBits=64)
  ...
  Sample_ID prefix convention (kept in sync with task1_data_curation.py):
    Polyimide             -> Po_XXXX   (reference)
    PEEK                  -> PE_XXXX   (reference)
    PTFE                  -> PT_XXXX   (reference)
    CyanateEster          -> CE_XXXX   (reference)
    FluorinatedPolyimide  -> FP_XXXX   (reference)
    Polybenzoxazole       -> PB_XXXX   (reference)
    <user polymer>        -> first 2 chars, upper-cased  (non-reference)
                             task4_output_management.py prepends 'USR_' to user
                             rows when joining into morgan_fingerprint_master.csv
                             (Requirement 5 of the v2 consolidation spec).

  Output schema (morgan_fingerprints.pkl):
    A pandas DataFrame with (N_polymers × 240) rows and 66 columns:
      - Polymer    : polymer class name (str)
      - Sample_ID  : matches task1 Sample_ID (str)
      - morgan_fp_00 ... morgan_fp_63 : 64 binary ECFP4 bits (uint8)

  Row count scales automatically with the number of registered polymers
  (6 reference polymers → 1 440 rows; more if user polymer is present).

  This schema is consumed by task4_output_management.py, which left-joins on
  [Polymer, Sample_ID] and appends desirability_score so chemical bit patterns
  of top-ranked designs are immediately visible in morgan_fingerprint_master.csv.
...
POLYMER_REGISTRY = {
    ...
    # ── 3 new reference polymers added to match Task 1 ────────────────────────
    "CyanateEster": {
    ...
    "FluorinatedPolyimide": {
        "smiles" : ("O=C1c2cc(C(F)(F)F)ccc2C(=O)N1c3ccc"
                    "(Oc4ccc(N5C(=O)c6cc(C(F)(F)F)ccc6C5=O)cc4)cc3"),
        "color"  : "\033[96m",
        "member" : "Member5",
    },
    "Polybenzoxazole": {
        "smiles" : "c1ccc2oc(-c3nc4ccccc4o3)nc2c1",
        "color"  : "\033[91m",
        "member" : "Member6",
    },
}
...
#   0 – 11 : Benzene ring substructures   (PI + PEEK + FPI + PBO, absent PTFE/CE)
#   12– 19 : General aromatic fragments   (PI + PEEK + FPI + PBO + CE partial)
#   20– 27 : Ether linkage substructures  (PI + PEEK + FPI + CE)
#   28– 35 : Imide ring substructures     (PI + FPI only)
#   36– 41 : Aryl-ketone substructures    (PEEK only)
#   42– 55 : CF2/CF3 fluorocarbon bits    (PTFE high; FPI moderate)
#   56– 63 : Misc aliphatic/connectivity  (all, low probability)
#
# Probability columns: PI  PEEK  PTFE  CE    FPI   PBO
_BIT_REGIONS = {
    # name             start  end   P_PI   P_PEEK  P_PTFE  P_CE   P_FPI  P_PBO
    "benzene"      : (  0,  12,  0.70,   0.75,   0.00,  0.40,  0.65,  0.80),
    "aromatic"     : ( 12,  20,  0.60,   0.50,   0.00,  0.35,  0.55,  0.75),
    "ether"        : ( 20,  28,  0.55,   0.62,   0.00,  0.60,  0.50,  0.00),
    "imide"        : ( 28,  36,  0.80,   0.00,   0.00,  0.00,  0.75,  0.00),
    "aryl_ketone"  : ( 36,  42,  0.08,   0.85,   0.00,  0.05,  0.06,  0.05),
    "fluorocarbon" : ( 42,  56,  0.00,   0.00,   0.70,  0.00,  0.35,  0.00),
    "misc"         : ( 56,  64,  0.14,   0.10,   0.08,  0.12,  0.13,  0.10),
}
...
def _mock_morgan_fingerprint(polymer_name: str) -> np.ndarray:
    ...
    Covers all 6 reference polymers (PI, PEEK, PTFE, CyanateEster,
    FluorinatedPolyimide, Polybenzoxazole). Unknown polymer names fall back to
    a SMILES-hash-seeded generic aromatic profile so they never crash or return
    all-zeros silently.
    """
    # col indices in _BIT_REGIONS tuples: 2=PI 3=PEEK 4=PTFE 5=CE 6=FPI 7=PBO
    prob_table = {
        ...
        "CyanateEster"        : {k: v[5] for k, v in _BIT_REGIONS.items()},
        "FluorinatedPolyimide": {k: v[6] for k, v in _BIT_REGIONS.items()},
        "Polybenzoxazole"     : {k: v[7] for k, v in _BIT_REGIONS.items()},
    }

    if polymer_name not in prob_table:
        # Generic fallback: moderate aromatic profile seeded on the name hash
        # so that different unknown polymers still get distinct fingerprints.
        name_seed = int(hashlib.md5(polymer_name.encode()).hexdigest(), 16) % (2**32)
        name_rng  = np.random.RandomState(name_seed)
        generic   = {k: float(name_rng.uniform(0.1, 0.6))
                     for k in _BIT_REGIONS}
        prob_table[polymer_name] = generic
...
def build_morgan_dataframe(fp_dict: dict) -> pd.DataFrame:
    """
    Build the output DataFrame that task4 expects.
    ...
    The same bit vector is replicated N_SAMPLES_PER_POLYMER times per polymer
    because the fingerprint is a property of the repeat-unit SMILES (fixed
    chemistry).  Sample_ID prefixes match task1_data_curation.py:
      Polyimide            -> 'Po_XXXX'
      PEEK                 -> 'PE_XXXX'
      PTFE                 -> 'PT_XXXX'
      CyanateEster         -> 'CE_XXXX'
      FluorinatedPolyimide -> 'FP_XXXX'
      Polybenzoxazole      -> 'PB_XXXX'
      <user polymer>       -> first two chars of name, upper-cased
    """
    # Explicit prefix map kept in sync with task1; unknown names get auto-prefix.
    PREFIX_MAP = {
        "Polyimide"           : "Po",
        "PEEK"                : "PE",
        "PTFE"                : "PT",
        "CyanateEster"        : "CE",
        "FluorinatedPolyimide": "FP",
        "Polybenzoxazole"     : "PB",
    }

    records = []
    for polymer_name, fp in fp_dict.items():
        prefix = PREFIX_MAP.get(polymer_name, polymer_name[:2].upper())
...
def print_fingerprint_report(fp_dict: dict, registry: dict) -> None:
    """Formatted Morgan fingerprint report with Tanimoto similarity matrix.

    Parameters
    ----------
    fp_dict  : {polymer_name: np.ndarray} — fingerprints for every polymer
               including any user polymer added at runtime.
    registry : the working registry dict used during this run (may contain
               more entries than the module-level POLYMER_REGISTRY constant
               when a user polymer has been added).
    """
    print_banner(
    ...
    # Per-polymer stats — iterate fp_dict so user polymer is always included
    print(f"\n  {'Polymer':<24}  {'SMILES (truncated)':<38}  "
          f"{'ON bits':>8}  {'Density':>9}  {'OFF bits':>9}")
    print(f"  {'─'*24}  {'─'*38}  {'─'*8}  {'─'*9}  {'─'*9}")
    for pname, fp in fp_dict.items():
        meta   = registry.get(pname, {"smiles": "N/A", "color": RESET})
        on     = int(fp.sum())
        smiles = meta.get("smiles", "N/A")
        smiles_short = smiles[:35] + "..." if len(smiles) > 35 else smiles
        color  = meta.get("color", RESET)
        print(f"  {color}{BOLD}{pname:<24}{RESET}  "
    ...
    print(f"  {'─'*24}  " + "  ".join(["─"*12] * len(names)))
    for a in names:
        meta  = registry.get(a, {"color": RESET})
        color = meta.get("color", RESET)
        row   = "  ".join(
            f"{tanimoto_similarity(fp_dict[a], fp_dict[b]):>12.4f}"
            for b in names
        )
        print(f"  {color}{BOLD}{a:<24}{RESET}  {row}")

    # Interpretation — build pairs dynamically from whatever is in fp_dict
    names = list(fp_dict.keys())
    pairs = []
    for i, a in enumerate(names):
        for b in names[i+1:]:
            # Derive a short chemical note for known relationships
            KNOWN_NOTES = {
                frozenset({"Polyimide", "PEEK"})            : "Both aromatic → shared substructures expected",
                frozenset({"Polyimide", "FluorinatedPolyimide"}): "Both imide-based → high structural overlap",
                frozenset({"Polyimide", "Polybenzoxazole"}) : "Both rigid-aromatic → moderate overlap",
                frozenset({"PEEK", "FluorinatedPolyimide"}) : "Aromatic ether vs fluorinated imide",
                frozenset({"Polyimide", "PTFE"})            : "Aromatic/imide vs fluorocarbon → near-zero overlap",
                frozenset({"PEEK", "PTFE"})                 : "Aromatic/ketone vs fluorocarbon → near-zero overlap",
                frozenset({"FluorinatedPolyimide", "PTFE"}) : "Fluorinated imide vs PTFE → partial fluorocarbon overlap",
                frozenset({"CyanateEster", "PTFE"})         : "Aromatic cyanate vs fluorocarbon → near-zero overlap",
                frozenset({"Polybenzoxazole", "PTFE"})      : "All-aromatic vs fluorocarbon → near-zero overlap",
            }
            note = KNOWN_NOTES.get(frozenset({a, b}), "Structural similarity")
            pairs.append((a, b, note))
    print(f"\n  Chemical interpretation:")
    for a, b, note in pairs:
        t = tanimoto_similarity(fp_dict[a], fp_dict[b])
        print(f"    {a} <-> {b:<22}: T = {t:.4f}  ({note})")
...
if __name__ == "__main__":
    ...
    # ── Load master dataset to pick up any user polymer rows ─────────────────
    print(f"\n  Loading master_dataset.pkl ...")
    with open("master_dataset.pkl", "rb") as f:
        master_df = pickle.load(f)
    print(f"  ✔  Loaded master_dataset.pkl  ({len(master_df)} rows)\n")

    # ── Optionally load user polymer metadata ─────────────────────────────────
    registry = dict(POLYMER_REGISTRY)   # working copy — may gain user polymer
    user_meta = None
    if os.path.exists("user_polymer.pkl"):
        with open("user_polymer.pkl", "rb") as f:
            user_meta = pickle.load(f)
        uname  = user_meta["name"]
        usmiles = user_meta.get("smiles", "")
        if uname not in registry and usmiles:
            registry[uname] = {
                "smiles": usmiles,
                "color" : "\033[97m",   # white for user polymer
                "member": "UserPolymer",
            }
            print(f"  ✔  User polymer '{uname}' added to registry for fingerprinting.\n")

    n_polymers = len(registry)
    print(f"  Computing ECFP{MORGAN_RADIUS*2} fingerprints "
          f"(radius={MORGAN_RADIUS}, nBits={MORGAN_BITS}) "
          f"for {n_polymers} polymer(s) ...\n")

    # ── Compute one fingerprint vector per polymer ────────────────────────────
    fp_dict = {}
    for pname, meta in registry.items():
        fp = compute_morgan_fingerprint(pname, meta["smiles"])
        fp_dict[pname] = fp
        print(f"  {meta['color']}{BOLD}{pname:<24}{RESET}  "
              f"ON bits: {int(fp.sum()):>3} / {MORGAN_BITS}  "
              f"(density = {fp.sum()/MORGAN_BITS:.3f})")

    # ── Build the output DataFrame in the schema task4 expects ────────────────
    morgan_df = build_morgan_dataframe(fp_dict)

    # ── Verify shape and schema (dynamic — no hard-coded row count) ───────────
    expected_rows = n_polymers * N_SAMPLES_PER_POLYMER
    expected_cols = 2 + MORGAN_BITS  # Polymer + Sample_ID + 64 bits
    assert morgan_df.shape == (expected_rows, expected_cols), (
        f"Shape error: {morgan_df.shape} "
        f"(expected ({expected_rows}, {expected_cols}) for "
        f"{n_polymers} polymer(s) × {N_SAMPLES_PER_POLYMER} samples)"
    )
    ...
    # ── Print report ──────────────────────────────────────────────────────────
    print_fingerprint_report(fp_dict, registry)
    ...
    print(f"  CSV format : pandas DataFrame  |  "
